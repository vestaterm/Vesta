...
| `vesta-link-hover` | true | URLs underline and the cursor turns into a hand when you point at one; `⌘`-click opens it. Inside apps that grab the mouse (Claude Code, vim) hold `⇧⌘` — `⇧` frees the pointer from the app, and faking it so plain hover worked there would put a phantom `⇧` in every wheel report the app receives. `false` = ghostty's own rule: `⌘`-hover in a normal pane, `⇧⌘` under an app that grabs the mouse |
| `vesta-link-click` | cmd | which click opens the link under the pointer: `cmd` (⌘-click, ghostty's own rule), `double`, or `single`. A real `⌘`-click opens in every mode (`⇧⌘` inside an app that grabs the mouse); the looser modes stand down under such an app, which would otherwise see a press with no release |
