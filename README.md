...
| `vesta-link-hover` | true | URLs underline and the cursor turns into a hand when you point at one; `⌘`-click opens it. inside apps that grab the mouse (Claude Code, vim) hold `⇧⌘` — `⇧` frees the pointer from the app, and faking it so plain hover worked there would put a phantom `⇧` in every wheel report the app receives. `false` = ghostty's own rule, `⌘`-hover everywhere |
