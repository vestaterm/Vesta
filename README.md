...
| `vesta-link-hover` | true | URLs underline and the cursor turns into a hand when you point at one; `⌘`-click opens it. `false` = ghostty's own rule, where a link only lights up while `⌘` is held — which is what applies inside apps that grab the mouse (Claude Code, vim) regardless, since making hover work there would put a phantom `⇧` in every wheel report |
