...
enum Worktree {
    /// Managed worktree root: ~/.vesta/worktrees/<repo>/<safe-branch>
...
extension Worktree {
    ...
    /// Managed root for all worktrees: ~/.vesta/worktrees
    static var defaultRoot: String {
        (NSHomeDirectory() as NSString).appendingPathComponent(".vesta/worktrees")
    }
    ...
    private static func run(_ tool: String, _ args: [String]) throws {
        ...
        if p.terminationStatus != 0 {
            ...
            throw NSError(domain: "vesta.worktree", code: Int(p.terminationStatus),
...
func worktreeSelfCheck() {
    ...
    let d = Worktree.dir(root: "/r", repo: "vesta", branch: "feat/x")
    assert(d == "/r/vesta/feat-x", "dir compose, got \(d)")
